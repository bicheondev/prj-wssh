...
type Host = { id: string; display_name: string; identity_id?: string };

export function TerminalView({ token, sessions, activeSessionId, hosts, scrollback, fontFamily, fontSize, identities }: { token: string; sessions: Session[]; activeSessionId?: string; hosts: Host[]; scrollback:number; fontFamily:string; fontSize:number; identities:any[] }) {
  ...
  useEffect(() => {
    sessions.forEach((s) => {
      ...
      const term = new Terminal({ scrollback, fontSize, fontFamily, theme: { background: '#081025', foreground: '#d7deed', cursor: '#69f0a1' } });
      ...
      ws.onopen = () => ws.send(JSON.stringify({ type: 'start', hostId: s.hostId, cols: term.cols, rows: term.rows, connectOnce: (s as any).connectOncePassword ? { secret: (s as any).connectOncePassword } : undefined }));
  ...
  }, [sessions, hosts, token, updateSession, scrollback, fontFamily, fontSize]);
