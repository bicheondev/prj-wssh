...
const v6blocked = [/^::1$/i,/^fe80:/i,/^ff/i,/^fc/i,/^fd/i];
...
export async function validateTarget(hostname: string, port: number, allowPrivate: boolean) {
  ...
  for (const a of addrs) {
    ...
    if (net.isIPv6(a.address) && !allowPrivate && v6blocked.some((r) => r.test(a.address))) throw new Error('Blocked by network policy');
  }
