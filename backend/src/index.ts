...
app.post('/api/hosts', auth, asyncWrap(async (req: Request & { userId?: string }, res: Response) => {
  await validateTarget(req.body.hostname, Number(req.body.port), env.ADMIN_ALLOW_PRIVATE_NETWORKS);
  const id = uuid(); const now = new Date().toISOString();
  db.prepare('INSERT INTO hosts VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)').run(id, req.userId, req.body.displayName, req.body.hostname, Number(req.body.port), req.body.sshUsername, req.body.groupName || null, req.body.favorite ? 1 : 0, req.body.notes || null, null, req.body.identityId || null, now, now);
  audit(req.userId!, 'host.create', id);
  res.status(201).json({ id });
}));
app.put('/api/hosts/:id', auth, asyncWrap(async (req: Request & { userId?: string }, res: Response) => {
  const r = db.prepare('UPDATE hosts SET display_name=?,hostname=?,port=?,ssh_username=?,group_name=?,favorite=?,notes=?,identity_id=?,updated_at=? WHERE id=? AND user_id=?')
    .run(req.body.displayName, req.body.hostname, Number(req.body.port), req.body.sshUsername, req.body.groupName || null, req.body.favorite ? 1 : 0, req.body.notes || null, req.body.identityId || null, new Date().toISOString(), String(req.params.id), req.userId);
  if (!r.changes) return res.status(404).json({ message: 'Not found' });
  audit(req.userId!, 'host.update', String(req.params.id)); res.status(204).end();
}));
app.delete('/api/hosts/:id', auth, asyncWrap(async (req: Request & { userId?: string }, res: Response) => {
  const r = db.prepare('DELETE FROM hosts WHERE id=? AND user_id=?').run(String(req.params.id), req.userId);
  if (!r.changes) return res.status(404).json({ message: 'Not found' });
  audit(req.userId!, 'host.delete', String(req.params.id)); res.status(204).end();
}));
app.get('/api/identities', auth, asyncWrap(async (req: Request & { userId?: string }, res: Response) => {
  res.json(db.prepare('SELECT id,name,type,created_at FROM identities WHERE user_id=? ORDER BY created_at DESC').all(req.userId));
}));
app.post('/api/identities', auth, asyncWrap(async (req: Request & { userId?: string }, res: Response) => {
  const id = uuid();
  db.prepare('INSERT INTO identities VALUES (?,?,?,?,?,?,?)').run(id, req.userId, req.body.name, req.body.type || 'privateKey', encryptSecret(req.body.secret, env.ENCRYPTION_KEY), req.body.passphrase ? encryptSecret(req.body.passphrase, env.ENCRYPTION_KEY) : null, new Date().toISOString());
  audit(req.userId!, 'identity.create', id); res.status(201).json({ id });
}));
app.delete('/api/identities/:id', auth, asyncWrap(async (req: Request & { userId?: string }, res: Response) => {
  const r = db.prepare('DELETE FROM identities WHERE id=? AND user_id=?').run(String(req.params.id), req.userId);
  if (!r.changes) return res.status(404).json({ message: 'Not found' });
  audit(req.userId!, 'identity.delete', String(req.params.id)); res.status(204).end();
}));
...
wss.on('connection', (ws: any, req: any) => {
  ...
  ws.on('message', async (raw: any) => {
    ...
    if (m.type === 'start') {
      const active = [...sessions.values()].filter((x) => x.userId === userId && x.status === 'connected').length;
      if (active >= env.MAX_SESSIONS_PER_USER) { ws.send(JSON.stringify({ type: 'state', state: 'failed', reason: 'session_limit' })); return; }
      try {
